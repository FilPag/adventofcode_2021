class Grid:
  ...
  def add_line(self, start, stop):
    ...
    if dx == 0:
      y_sign = int(abs(y2 - y1) / (y2 - y1))

      for y in range(dy + 1):
        self._add_to_grid(x1, y1 + y_sign * y)

    elif dy == 0:
      x_sign = int(abs(x2 - x1) / (x2 - x1))

      for x in range(dx + 1):
        self._add_to_grid(x1 + x_sign * x , y1)
    elif dx == dy:
      x_sign = int(abs(x2 - x1) / (x2 - x1))
      y_sign = int(abs(y2 - y1) / (y2 - y1))

      for i in range (dx + 1):
        self._add_to_grid(x1 + x_sign * i , y1 + y_sign  * i)
